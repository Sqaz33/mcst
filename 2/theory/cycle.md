...
Массив, header, где header[w] = хедер цикла, к которому принадлежит w.

## Алгоритм из статьи Havlak P - analyze_loops.

### UNION, FIND

Ncf - узлы из cfg

i - номер узла Mi в прямом порядке DFS, от 1 до N.

U = Ncf 

Есть множества Si, которые принадлежат U.

Изначально Si = {Mi}

Операции объединение-поиск

* UNION(x, y) - все узлы из Sx в Sy. Sx - пустое.
* FIND(x) - если Sy = {..., x, ...}, то FIND(x) = y.

### Алгоритм в analyze_loops:

a: пронумеровать узлы в преордере

b: идти по узлам в прямом порядке, смотреть для каждого узла входящие ребра v->w, и если источник v является потомком w, то классифицировать v->w как обращенное ребро

с: идти по cfg в обратном нумерации, берем узел w. P будет телом  цикла (без хедера), пока пустое.

d: берем источики обращенных ребер v->w. Если это не петля, то добавляем FIND(v) в P (заполняем тело цикла)

e:

* worklist = P. worklist.pop() -> x пока worklist не пуст.
* берем узлы-источники y из "не обращенных входящих в x ребер"
* y' = FIND(y)
* Если y' не потомок w, то добавляем y' в спиосок "не обращенных входящих в w ребер". Иначе если x не в P и не равен w - добавляем x в P и в worklist
* Для всех узлов x из P ставим header[x] = w и UNION(x, w)

### header[]

hedear - древовидная структура в массиве, где header[w] - указывает на хедер цикла в котором узел w, если w сам хедер, то на хедер внешнего цикла
