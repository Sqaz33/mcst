...
d: берем источик обратного ребра v->w, если это не петля то добавляем FIND(v) в P (заполняем тело цикла)

e:

* worklist = P. worklist.pop() -> x пока worklist не пуст.
* Если x не в P и не равен w - добавляем x P и в worklist
* Для всех узлов x из P ставим header[x] = w и UNION(x, w)

hedear - древовидная структура в массиве, где header[w] - указывает на хедер узла w, если w сам хедер, то на хедер внешнего цикла
